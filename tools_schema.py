...
MASTER TABLE - Returns Test Code + Matrix Code needed for lookup tools.

USE WHEN:
- User asks about a parameter or test (detection limits, reporting limits, test codes, methods)
- User references a test code directly (e.g., F2648, F2202)
- User asks "What test do I need for [parameter]?"
...
PREREQUISITE:
Get test codes first using search_testportfolio for each parameter.
...
WORKFLOW:
1. Call search_testportfolio for each parameter → get test_codes
...
1. search_testportfolio("iron groundwater") → test_code: "F1068"
2. search_testportfolio("manganese groundwater") → test_code: "F1070"
3. search_testportfolio("sodium groundwater") → test_code: "F1073"
            ...
            "parameters": {
                ...
                "properties": {
                    "test_codes": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "List of test codes to analyze (get from search_testportfolio)"
                    },
                    "matrix": {
                        "type": "string",
                        "description": "User's requested matrix (e.g., 'groundwater', 'soil'). LLM will check if this matrix exists and inform user of available options if not found."
                    }
                },
                "required": ["test_codes"]
            }
