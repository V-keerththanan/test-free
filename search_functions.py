...
# =============================
# PARAMETER/TEST SEARCH FUNCTION (DEPRECATED - Commented out)
# =============================
# This function added extra complexity by requiring multiple tool calls.
# Use search_testportfolio directly instead.
...
def get_container_requirements_for_tests(test_codes: list, matrix: str = None) -> dict:
    ...
    1. Fetches ALL logistics data for each test code (no matrix filtering)
    2. Sends data to GPT for analysis with user's requested matrix
    3. GPT determines sharing opportunities, handles matrix mismatch, and explains to user
    """
    import json

    # 1. Fetch ALL logistics for each test code (no matrix filtering - let LLM handle it)
    test_data = []
    tests_not_found = []
    ...
    for test_code in test_codes:
        ...
        if logistics and not isinstance(logistics, str):
            # Add ALL rows for this test code (all matrices)
    ...
    user_matrix = matrix if matrix else "Not specified"

    prompt = f"""Analyze these test logistics and determine bottle sharing opportunities.

USER REQUESTED MATRIX: {user_matrix}

TEST DATA:
{json.dumps(data_for_llm, indent=2)}

IMPORTANT - MATRIX HANDLING:
1. If user matrix is "Not specified" →
   - Use DEFAULT or first available matrix for each test code
   - Proceed with bottle analysis normally (no need to ask user)
2. If user specified a matrix ("{user_matrix}") →
   - Check if it matches any "matrix" field in the data
   - If YES → Use only the matching matrix rows for bottle analysis
   - If NO (matrix mismatch) →
     - List ALL available matrices from the data
     - Clearly state: "Requested matrix '{user_matrix}' not found for these test codes"
     - Show what matrices ARE available
     - Still perform bottle analysis for available matrices so user knows options
...
{{
  "user_requested_matrix": "{user_matrix}",
  "matrix_found": true/false,
  "available_matrices": ["<list of all unique matrices in data>"],
  "matrix_used_for_analysis": "<which matrix was used>",
  "matrix_message": "<message to user about matrix match/mismatch>",
  "total_bottles_needed": <number or null if matrix mismatch>,
  "bottles_without_sharing": <number or null>,
  "bottles_saved": <number or null>,
  "container_groups": [
    {{
      "amt_code": "<code>",
      "amt_name": "<name>",
