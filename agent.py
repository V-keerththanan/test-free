...
### STEP 1: GET TEST CODE (when user mentions a parameter/test name)
- **search_testportfolio(search_query)**: Search by parameter name + matrix
  → Returns: Test Code, parameter name, LOD, LOQ, reporting limits, units, methods
  → This is the MASTER TABLE - get Test Code here first
  → If no match found → inform user and ask for clarification

### STEP 2: USE LOOKUP TOOLS (with Test Code from Step 1)
...
### WORKFLOW:
1. search_testportfolio for EACH parameter with matrix
2. Extract test_code from results
...
### MULTIPLE TEST CODES:
- If multiple test codes found for same parameter:
...
### WORKFLOW:
1. Call search_testportfolio for EACH parameter with matrix to get test_codes
   - Example: search_testportfolio("iron groundwater") → test_code: "F1068"
...
- ❌ Answer technical questions without calling tools first
...
2. For parameter names → search_testportfolio with parameter + matrix → then lookup tools with test_code
3. For test codes (F-codes) → search_testportfolio or lookup tools directly
            ...
            injected_messages = messages + [
                {
                    ...
                    "content": (
                        ...
                        "PHASE 1 - DISCOVERY: Use search_testportfolio to get Test Code from parameter name + matrix\n"
                        ...
                        "ABBREVIATION HANDLING:\n"
                        "When user provides abbreviations, use your chemistry knowledge to interpret them.\n"
                        "If uncertain, confirm with user BEFORE searching.\n\n"
                        "CRITICAL RULES:\n"
                        "- search_testportfolio is the MASTER TABLE — start here for parameter queries\n"
